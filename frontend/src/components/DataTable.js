...
const DataTable = ({ data, columns, totalRecords, onShowAnalysis }) => {
  ...
  const [dateFilterMode, setDateFilterMode] = useState('exact'); // 'exact' or 'range'
  const [selectedDate, setSelectedDate] = useState('');
  const [startDate, setStartDate] = useState('');
  const [endDate, setEndDate] = useState('');
  const [dateColumn, setDateColumn] = useState('');
  const [orderTypeFilter, setOrderTypeFilter] = useState(''); // '1', '2', '3', or ''
  const [itemServiceFilter, setItemServiceFilter] = useState(''); // 'Item' or 'Service' or ''
  const [paymentTermsFilter, setPaymentTermsFilter] = useState(''); // Payment terms filter
  const [paymentTermsSearch, setPaymentTermsSearch] = useState(''); // Search term for payment terms dropdown
  const [showPaymentTermsSuggestions, setShowPaymentTermsSuggestions] = useState(false); // Show/hide suggestions
  const [columnSearchTerms, setColumnSearchTerms] = useState({}); // Search terms for each column
  const [activeSearchColumn, setActiveSearchColumn] = useState(null); // Currently focused search column
  ...
  const parseDate = (dateStr) => {
    ...
    if (parts.length === 3) {
      ...
      // Handle 2-digit years (assume 20xx if < 100)
      if (third < 100) {
        third = 2000 + third;
      }
  ...
  const filteredData = useMemo(() => {
    ...
    // Apply column filters (exact match for dropdowns)
    ...
    // Apply column search filters (partial match for search inputs)
  ...
  const applyDateFilter = () => {
    if (!dateColumn) {
      alert('⚠️ Please select a date column first!');
      return;
    }
    if (dateFilterMode === 'exact' && !selectedDate) {
        ...
        <div className="order-type-filter">
          ...
          <div className="order-type-buttons">
            <button
              ...
              title="Filter Import Orders (Item codes starting with 1)"
            >
            ...
            <button
              ...
              title="Filter Local Orders (Item codes starting with 2)"
            >
            ...
            <button
              ...
              title="Filter Job Orders (Item codes starting with 3)"
            >
          ...
          {orderTypeFilter && (
            <div className="active-order-filter">
              ...
              Showing {orderTypeFilter === '1' ? 'Import' : orderTypeFilter === '2' ? 'Local' : 'Job'} Orders only
                  ...
                  <div className="th-content">
                    ...
                    {column === itemCodeColumn ? (
                      ...
                      >
                        ...
                        <option value="1">🔵 Import Orders ({orderTypeCounts['1']})</option>
                        <option value="2">🟢 Local Orders ({orderTypeCounts['2']})</option>
                        <option value="3">🟠 Job Orders ({orderTypeCounts['3']})</option>
                      </select>
